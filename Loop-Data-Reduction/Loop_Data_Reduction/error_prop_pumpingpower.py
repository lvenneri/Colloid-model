...
def function_to_exp(xs):
    delP = 26*L_t/A_c/d_hp * np.divide(np.multiply(xs[:,0],xs[:,2]),xs[:,1])
    return np.divide(np.multiply(delP,xs[:,0]),xs[:,1])
...
def single_exp2(mean = [40,1000,0.00089000],errors_frac = [.002,.01,.05],population= 1000):
    """

    :param mean:  mass flow, density, pressure drop
    :param errors_frac:
    :param population:
    :return:
    """
